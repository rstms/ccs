...
@click.pass_context
def ccs(ctx, region, username, password, debug):
    """CLI for CloudSigma
...
@ccs.group(name='list')
@click.option('-d', '--detail', is_flag=True, help='include extra detail')
@click.option('-u', '--uuid', is_flag=True, help='uuid only')
@click.option('-h', '--human', is_flag=True, help='human readable')
@click.pass_context
def list(ctx, detail, uuid, human):
...
def servers(ctx):
    """list servers"""
    output(ctx.obj.list_servers(ctx.obj.detail, ctx.obj.uuid, ctx.obj.human))

@list.command()
@click.pass_context
def drives(ctx):
    """list drives"""
    output(ctx.obj.list_drives(ctx.obj.detail, ctx.obj.uuid, ctx.obj.human))

@list.command()
@click.pass_context
def vlans(ctx):
    """list VLANs"""
    output(ctx.obj.list_vlans(ctx.obj.detail, ctx.obj.uuid, ctx.obj.human))

@list.command()
@click.pass_context
def ips(ctx):
    """list IPs"""
    output(ctx.obj.list_ips(ctx.obj.detail, ctx.obj.uuid, ctx.obj.human))

@list.command()
@click.pass_context
def subscriptions(ctx):
    """list subscriptions"""
    output(ctx.obj.list_subscriptions(ctx.obj.detail, ctx.obj.uuid, ctx.obj.human))

@list.command()
@click.pass_context
def capabilities(ctx):
    """list capabilities"""
    output(ctx.obj.list_capabilities(ctx.obj.detail, ctx.obj.uuid, ctx.obj.human))

@list.command()
@click.pass_context
def all(ctx):
    """list all"""
    output(ctx.obj.list_all(ctx.obj.detail, ctx.obj.uuid, ctx.obj.human))
